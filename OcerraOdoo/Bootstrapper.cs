...
namespace OcerraOdoo
{
    public class Bootstrapper : DefaultNancyBootstrapper
    {
        ...
        protected override void ApplicationStartup(TinyIoCContainer container, IPipelines pipelines)
        {
            ...
            var connectToOcerra = container.Resolve<OcerraClient>().ApiClientCurrentGetAsync();
            var connectToOdoo = container.Resolve<OdooRpcClient>().GetOdooVersion();
            
            try
            {
                if (Task.WaitAll(new[] { (Task)connectToOcerra, connectToOdoo }, TimeSpan.FromSeconds(30)))
                {
                    OcerraModel = new OcerraModel
                    {
                        ClientId = connectToOcerra.Result.ClientId,
                        ClientName = connectToOcerra.Result.Name,
                        CountryCode = connectToOcerra.Result.CurrencyCodes?.FirstOrDefault(cc => cc.IsDefault)?.CountryCode,
                        CurrencyCode = connectToOcerra.Result.CurrencyCodes?.FirstOrDefault(cc=>cc.IsDefault)?.Code,
                        CurrencyCodes = connectToOcerra.Result.CurrencyCodes?.ToList(),
                        Connected = true,
                    ...
                    {
                        Connected = true,
                        VersionNumber = connectToOdoo.Result.ServerVersion
                    };
                }
            } 
            catch(Exception ex)
